...
//				System.out.println("Already selected option name: "+selectedOptionName);
				...
				Select monthDrop=new Select(driver.findElement(By.className("form-control ng-pristine ng-invalid ng-invalid-required ng-touched")));
				List<Integer> month=new ArrayList<Integer>();
				for(int i=1;i<monthDrop.getOptions().size();i++) {
					String day=monthDrop.getOptions().get(i).getText();
					days.add(Integer.parseInt(day));
				}
				List<Integer> Dropmonth=new ArrayList<Integer>(month);
				//dupDays.addAll(days);
				//Collections.sort(dupDays);
				System.out.println(days.equals(Dropmonth));
